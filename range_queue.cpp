	...
	while( it != _range_q.end() )
	{
		...
		else
		{
			if( it->end() >= next_it->pos() )
			{
				...
				_range_q.erase( next_it );//ɾ���Ժ�next_it �����¸�ֵ�ˣ��ǰ�ȫ��
			}
			else
				++it;
		}
...
const std::vector<range>& range_queue::operator -= (const range& r)
{
	...
	if( 0 != r.length() )
	{
		...
		if(it_find != _range_q.begin())
		{
			...
			{//can be excised
				uint64_t old_end = (it_find - 1)->end();
				(it_find - 1)->set_length(r.pos() - (it_find - 1)->pos());
				
				if((it_find - 1)->length() == 0)
				{
					it_find = _range_q.erase(it_find - 1);//it_find ��ǰһ��Ԫ�ر�ɾ���ˣ�it_find��λ����Ҫǰ��һλ
					//it_find --;
				}
...
const std::vector<range>& range_queue::operator += ( const range_queue& r_q)
{
	...
	swap( temp_queue );
#else
	#pragma message( "�ڲ���range_queue+=����ʵ�֣����û�����⣬�ڲ�Ʒ����������Ҫɾ����Դ���" )
...
void range_queue::add_new_to(const range_queue &r_q, range_queue &add_q, range_queue &new_q)
{
	bool has_merged = false;
	bool src_been_trunced = false;
	uint64_t merged_begin_pos, trunced_pos;
...
uint64_t range_queue::all_range_length(void) const
{
	uint64_t ret_val = 0;
...
bool range_queue::is_complete_range(const range& r, uint64_t file_size) const
{
	std::vector<range>::const_iterator it_find = std::lower_bound(_range_q.begin(), _range_q.end(), r);
...
void range_queue::to_string( std::string & result ) const
{
	...
	{
		sprintf( buf,"(%u)[%llu, %llu, %llu)%s", i+1, _range_q[i].pos(), _range_q[i].length(), _range_q[i].end(), 
			i<(_range_q.size()-1)?"\r\n":"" );
		result += buf;
	}
}
...
void range_queue::inc_offset( uint64_t offset_value )
{
...
void range_queue::dec_offset( uint64_t offset_value )
{
	if ( false == _range_q.empty() && offset_value )
	{
		std::vector<range>::iterator it = _range_q.begin();
...
void range_sub_range_queue( const range& r, const range_queue& minuend, range_queue& result )
{
	...
	if( 0 != r.length() )
	{
		...
		uint64_t cur_pos = r.pos();
		uint64_t cur_length = r.length();
		...
		while (it_find != m_vector.end() && it_find->pos() < r.end())
		{
